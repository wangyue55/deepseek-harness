...
公司内网工具的 profile bundle:一个 patch 层,把 [`dsh-intranet-tool-wiki`](../../intranet/tool-wiki/README.md) 与 [`dsh-intranet-tool-gitlab`](../../intranet/tool-gitlab/README.md) 的行插入到 `dsh-base` 之上。
...
把 bundle 加入 profile 的叠层列表([profile 契约](../../boot/app-boot/README.md#profiles)),或用 `dsh plugin --profile <name> add @deepseek-ai/dsh-intranet` 在仓外安装。调用工具前导出四个引用值(或存入凭证服务);缺失时调用失败并给出补救指引。开发期用 `pnpm dsh web --patch packages/bundle/intranet/cordis.patch.yml` 在 web profile 上挂载同样的行。
